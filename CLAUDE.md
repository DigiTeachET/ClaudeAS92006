...
Doors, the companion NPC, and the journal object are all `Area2D` nodes distinguished by a `metadata/kind` string (`"door" | "companion" | "journal"`) set directly in the `.tscn` file rather than in code — `Hub.gd` reads `area.get_meta(...)` to decide what interacting with a given object does. Door unlock state is visualized by toggling a `Locked` overlay child node per-door in `_refresh_doors()`, driven by `ProgressTracker.is_mode_unlocked(mode_id)`.

The hub is a chain of six rooms (one per mode plus an entry room) joined by narrow corridors, not one open area. `Hub.gd`'s `ROOMS` constant is the single source of truth for room order/width/label/colour; `_build_level()` derives the corridor rects in the gaps between rooms and generates all floor visuals at runtime into the `Floors` node — nothing about room layout is hand-authored in the `.tscn` beyond doors/companion/journal being placed at coordinates that line up with the generated room centres (see the comment above `ROOMS`; if you change room widths, the hand-placed positions need updating to match). There's still no physics body: movement is a plain `Node2D` position update, but each frame's move is only applied per-axis if the resulting point falls inside one of the generated room/corridor `Rect2` cells (`_in_any_cell`) — that's what makes an unaligned corridor doorway block the player like a wall, without needing collision shapes for movement. A `Camera2D` on the player (with `limit_*` set to the level bounds) follows the player so only one room/corridor is visible at a time, rather than showing the whole map. The `Area2D` "Detector" child is unrelated to this - it's only used for proximity overlap detection (interact prompts), not collision.
